# pod repo push REPO_NAME SBLibraries.podspec --use-libraries --allow-warnings
...
Pod::Spec.new do |s|
  ...
  # This added for facebook sdk v4 compatability, these pods are still compatable with ios 6 projects
  s.platform     = :ios, '7.0'
  s.source       = { :git => "https://github.com/jibjab/storybots-libraries.git", :tag => "1.2.0" }
  s.requires_arc = true
  ...
  s.subspec 'OneHeadCastingFunnel' do |ss|
    ss.source_files = "CastingFunnel/SBCastingFunnel/Classes/{SBOneHeadAddPhotoViewController,"\
                      "SBOneHeadPositionFaceViewController,SBPositionFaceTipViewController,"\
                      "SBMaskedImageViewController}.{h,m}"
    ss.resources = "CastingFunnel/SBCastingFunnel/Resources/*.png"
    ss.dependency 'SBLibraries/Utilities'
  ...
  s.subspec 'Tracking' do |ss|
    ...
    ss.dependency 'Facebook-iOS-SDK', '~> 4.0'
    ss.dependency 'FiksuSDK', '~> 4.4'
    ss.xcconfig = { "FRAMEWORK_SEARCH_PATHS" => "\"$(PODS_ROOT)/FiksuSDK/**\"" }
    ss.dependency 'GoogleAnalytics-iOS-SDK', '~> 3.10'
    ss.dependency 'FlurrySDK', '~> 6.2'
  end
